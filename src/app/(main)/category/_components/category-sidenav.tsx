            ...
            <div key={category.tab} className="flex flex-col gap-3 mt-5">
              ...
              {category.tab === "CATEGORY" && isLoading ? (
                ...
                <>
                  {category.options[0] &&
                    (() => {
                      const AllItemsIcon = category.options[0]
                  ...
                  {categoriesData.map((apiCategory) => {
                    ...
                    const StaticIcon = staticCategory?.Icon as unknown as
                      | React.ComponentType<any>
                      | undefined;
                    return (
                      <Link
                        key={apiCategory.id}
                        href={`?tab=${staticCategory?.path || apiCategory.id}&q=${apiCategory.id}`}
                        className={`flex items-center gap-2 py-2 my-1 px-2 ${
                ...
                category.options.map(({ label, path, Icon }) => {
                  ...
                  return (
                    <Link
                      ...
                      href={`?tab=${path}`}
                      className={`flex items-center gap-2 py-2 my-1 px-2 ${
                        path === tab ? "bg-[#F4F4F4] rounded-xl border border-[#E3E3E3]" : ""
                      }`}
                    >
